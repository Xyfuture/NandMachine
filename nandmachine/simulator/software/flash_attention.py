...
import copy
from nandmachine.simulator.software.scalesim_runtime_patch import (
...
def _simulate_flashattn_qk_cli_read_cycle_count(
    ...
    *,
    ...
    if qk_mk_bytes == 0 and qk_kn_bytes == 0:
        return 0
    if qk_mk_bytes == 0:
        return _bytes_to_cycle_count_or_raise(
            qk_kn_bytes,
            qk_kn_bandwidth_per_cycle,
        )
    if qk_kn_bytes == 0:
        return _bytes_to_cycle_count_or_raise(
            qk_mk_bytes,
            total_bandwidth_per_cycle
            if ENABLE_FLASHATTN_CLI_HBF_SRAM_BUFFER
            else hbm_bandwidth_per_cycle,
        )

    qk_mk_time_hbm = qk_mk_bytes / hbm_bandwidth_per_cycle
    qk_kn_time_hbf = qk_kn_bytes / qk_kn_bandwidth_per_cycle

    if ENABLE_FLASHATTN_CLI_HBF_SRAM_BUFFER:
        if qk_kn_time_hbf >= qk_mk_time_hbm:
            return ceil(qk_kn_time_hbf)
        return ceil((qk_mk_bytes + qk_kn_bytes) / total_bandwidth_per_cycle)

    return ceil(max(qk_mk_time_hbm, qk_kn_time_hbf))
...
class FlashAttn_BatchedMatMul_Simulation:
    ...
    def _compile_and_simulate_result(
        ...
        if matmul_cycle_count1 <= matmul_cycle_count2:
            best_mapping = "per_batch"
            best_cycle_count = matmul_cycle_count1
        else:
            best_mapping = "merged_batch"
            best_cycle_count = matmul_cycle_count2
        return self.CompileResult(
            best_mapping=best_mapping,
            best_cycle_count=best_cycle_count,
            best_time_ns=_cycle_count_to_time_ns(best_cycle_count, pcb_module),
        )
    ...
    def compile_and_simulate(self,
        ...
        self.best_cycle_count = result.best_cycle_count
        self.best_time_ns = result.best_time_ns
        self.best_latency = result.best_cycle_count / pcb_module.compute_module.clock_freq
        self.latency = self.best_latency
        if return_unit == "cycle":
            return result.best_cycle_count
...
class MatMul_Simulation: # MNK指M*K的矩阵与K*N的矩阵相乘，输出M*N的矩阵
    ...
    @staticmethod
    def _get_core_vector_flops_per_cycle(device: Device, weight_bits: int) -> int:
        return device.compute_module.core.vector_unit.get_total_vector_flops_per_cycle(
            weight_bits
        )

    @staticmethod
    def _get_total_vector_flops_per_cycle(device: Device, weight_bits: int) -> int:
        return device.compute_module.get_total_vector_flops_per_cycle(weight_bits)

    @staticmethod
    def _get_array_mac_per_cycle(device: Device, weight_bits: int) -> int:
        return device.compute_module.core.systolic_array.get_mac_per_cycle(weight_bits)
    ...
    def __init__(
        ...
        self.weight_bits = weight_bits
        self.word_size = _word_size_from_weight_bits(weight_bits)
        self.precision = self.PrecisionContext(
    ...
    class ComputationalGraph:
        def __init__(
            ...
            self.word_size = precision.word_size
    ...
    def _build_compile_result(
        ...
        return self.CompileResult(
            best_mapping=best_mapping,
            best_cycle_count=min_cycle_count,
    ...
    def simulate(
        ...
        precision = computational_graph.precision
        ...
        for m, n, k in self.generate_tile_loops(
            ...
            if m == previous_m and k == previous_k:
                should_read_qk_mk = False
                should_read_qk_kn_or_sv_nk = True
            elif n == previous_n and k == previous_k:
                should_read_qk_mk = True
                should_read_qk_kn_or_sv_nk = False
            else:
                should_read_qk_mk = True
                should_read_qk_kn_or_sv_nk = True
            current_tile_read_cycle_count = l2_tile.get_main_memory_read_cycle_count(
                read_qk_mk=should_read_qk_mk,
                read_qk_kn_or_sv_nk=should_read_qk_kn_or_sv_nk,
            )
            ...
            previous_tile_write_cycle_count = previous_l2_tile.get_main_memory_write_cycle_count(
                write_sv_mk=not (m == previous_m and n == previous_n)
            )
        ...
        total_cycle_count += (
            l2_tiles[-1, -1, -1].get_main_memory_write_cycle_count(write_sv_mk=True)
    ...
    class L2TileSimulator:
        def __init__(
            ...
            effective_total_vector_flops_per_cycle = (
                MatMul_Simulation._get_total_vector_flops_per_cycle(
                    pcb_module, precision.weight_bits
                )
            )
            self.K_reduction_cycle_count = ceil(
                M * N / effective_total_vector_flops_per_cycle
            ) # K规约也是完成于l1，因此此处没有l2与主存的io时间
            self.K_reduction_io_count = 2 * M * N * word_size
            self.qk_mk_read_bytes = M * K * word_size
            self.qk_kn_or_sv_nk_read_bytes = K * N * word_size
            self.sv_mk_write_bytes = M * N * word_size
            ...
            *,
            ...
            return _simulate_flashattn_main_memory_read_cycle_count(
                ...
                qk_kn_or_sv_nk_bytes=(
                    self.qk_kn_or_sv_nk_read_bytes if read_qk_kn_or_sv_nk else 0
                ),
            )
        ...
        def simulate_l2_tile_compute_cycle_count(
            ...
            word_size = precision.word_size
            effective_vector_flops_per_cycle = (
                MatMul_Simulation._get_core_vector_flops_per_cycle(
                    chiplet_module, precision.weight_bits
                )
            )
            # 在L2 tile内再做L1切分
    ...
    class L1TileSimulator:
        def __init__(
            ...
            self.compute_cycle_count = self.simulate_l1_tile_compute_cycle_count(
                M, N, K, precision, mapping, chiplet_module, look_up_table
            )
        ...
        def simulate_l1_tile_compute_cycle_count(
            ...
            compute_cycle_count = ceil(
                MatMul_Simulation.simulate_systolic_array_cycle_count(
                    ...
                    MatMul_Simulation._get_array_mac_per_cycle(
                        chiplet_module, precision.weight_bits
                    ),
                    mapping.dataflow,
                )
...
class Softmax_Simulation:
    ...
    class ComputationalGraph:
        def __init__(
            ...
            M: int,
            N: int,
            precision: "Softmax_Simulation.PrecisionContext",
        ):
            self.M = M
            self.N = N
            self.precision = precision
    ...
    def _build_compile_result(
        ...
        self.computational_graph.precision = self.precision
        self.computational_graph.word_size = self.precision.word_size
    ...
    class L2TileSimulator:
        def __init__(
            ...
            self.N = N
            self.compute_cycle_count = self.simulate_l2_tile_compute_cycle_count(
                M, N, precision, word_size, mapping, pcb_module
            )
    ...
    class L1TileSimulator:
        def __init__(
            ...
            self.effective_vector_flops_per_cycle = (
                Softmax_Simulation._get_core_vector_flops_per_cycle(
                    pcb_module, precision.weight_bits
                )
            )
